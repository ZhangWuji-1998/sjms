...
/* 分割点集 */
void distribute_points(const int N, const std::vector<Point3d>& points_3d, std::vector<std::vector<Point3d>>& distr_points) {
    // 根据点数确定分割的块数
    int block_num = 1;
    if (N < 5000) block_num = 1;
    else if (N < 7000) block_num = 4;
    else
        block_num = 8;
    // 对区域进行分割
    distr_points.resize(block_num);
    switch (block_num) {
    case 1:
        for (auto& p : points_3d) {
            distr_points[0].emplace_back(p);
        }
        break;
    case 4:
        for (auto& p : points_3d) {
            if (p.z > 0 && p.x > 0) distr_points[0].emplace_back(p);
            else if (p.z > 0 && p.x < 0) distr_points[1].emplace_back(p);
            else if (p.z < 0 && p.x > 0) distr_points[2].emplace_back(p);
            else
                distr_points[3].emplace_back(p);
        }
        break;
    case 8:
        for (auto& p : points_3d) {
            if (p.z > 0 && p.x > 0 && p.y > 0) distr_points[0].emplace_back(p);
            else if (p.z > 0 && p.x > 0 && p.y < 0) distr_points[1].emplace_back(p);
            else if (p.z > 0 && p.x < 0 && p.y > 0) distr_points[2].emplace_back(p);
            else if (p.z > 0 && p.x < 0 && p.y < 0) distr_points[3].emplace_back(p);
            else if (p.z < 0 && p.x > 0 && p.y > 0) distr_points[4].emplace_back(p);
            else if (p.z < 0 && p.x > 0 && p.y < 0) distr_points[5].emplace_back(p);
            else if (p.z < 0 && p.x < 0 && p.y > 0) distr_points[6].emplace_back(p);
            else
                distr_points[7].emplace_back(p);
        }
        break;
    default:
        break;
...
int main(int argc, char* argv[]) {
    ...
    std::vector<std::vector<Point3d>> distr_points;    // 分割后的三角形点集
    if (!part_params_load(N, k_input_filename, points_3d))return 0;    // 数据载入
    distribute_points(N, points_3d, distr_points);    // 根据需求分割三角形点集
    for (int i = 0; i < static_cast<int>(distr_points.size()); i++) {
        std::vector<Triangle> distr_triangles;  // 分割后的三角形集
        creat_tri_net(distr_points[i].size(), distr_points[i], distr_triangles);    // 生成局部三角网
        triangles.insert(triangles.cend(), distr_triangles.cbegin(), distr_triangles.cend());
    }
